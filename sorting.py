...
def run_lab_requirements():
    ...
    print("\nResult 20000 elements")
    data_20k = [random.randint(1, 1000000) for _ in range(20000)]

    print("Before sorting (top 20):")
    print(*(data_20k[:20]))

    s3 = time.time()
    sorted_m = merge_sort(data_20k.copy())
    print("After MergeSort (the first 20):")
    print(*(sorted_m[:20]))
    print(f"MergeSort runtime (20k): {time.time() - s3:.4f} сек")

    s4 = time.time()
    sorted_q = quick_sort(data_20k.copy())
    print("After QuickSort (the first 20):")
    print(*(sorted_q[:20]))
    print(f"QuickSort runtime (20k): {time.time() - s4:.4f} сек")
